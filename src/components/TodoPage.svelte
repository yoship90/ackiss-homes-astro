<script lang="ts">
  ...
  const BASE_ENTRIES: Entry[] = [
    ...
    { id: "meta-business-manager", title: "Meta Business Manager + Pixel ID — Jeremy", description: "Go to business.facebook.com, log in or create a Business Manager account, then go to Events Manager → Connect Data Sources → Web → Meta Pixel. Create a Pixel named 'Ackiss Homes' and share the Pixel ID with Nate so it can be installed on the site. See the Ads Guide for full context.", date: "Mar 2026", status: "completed", priority: "high", link: "/ads" },
    { id: "meta-pixel-install",   title: "Install Meta Pixel on Site", description: "Meta Pixel (ID: 886261974458529) installed on all pages. Automatic Advanced Matching enabled. Privacy Policy updated to disclose GA4 and Meta Pixel usage.", date: "Mar 2026", status: "completed", link: "/ads" },
    { id: "meta-conversions-api", title: "Meta Conversions API (Server-Side Tracking)", description: "Upgrade Meta tracking by adding the Conversions API alongside the existing browser pixel. The Conversions API sends events directly from the server to Meta, bypassing iOS privacy restrictions and ad blockers that can block the browser pixel. This improves match rates and ad performance data accuracy — especially important once running paid campaigns. Requires a Meta system user token and adding server-side event calls to the /api/lead route when a form is submitted (a 'Lead' event). Do this before spending seriously on Meta ads.", date: "Mar 2026", status: "todo", link: "/ads" },
